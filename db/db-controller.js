...
var getAllQuests = function(characterId) {
	return new Promise(function(resolve, reject) {
		return connection.queryAsync('SELECT q.id, q.experience, q.name, q.creator_id, q.lat, q.lng, q.questType, q.complete, c.active FROM Quests q LEFT OUTER JOIN CharacterQuests c ON (q.id = c.quest_id AND c.character_id = ' + characterId +') WHERE q.creator_id <> ' + characterId).then(function(result) {
